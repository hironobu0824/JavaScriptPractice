...
function quizStart() {
    ...
    function generate_table(quiz, result) {
        // body の参照を取得
        let quizResultText = document.getElementById("quiz-result-text");
        ...
        generate_row(tblBody, '問題', '解答', '正誤', 'th');
    
        // すべてのセルを作成
        for (let i = 0; i < quiz.length; i++) {
            if (result[i]['bool'] === true) {
                generate_row(tblBody, quiz[i]['question'], result[i]['answer'], "◯", 'td');
            } else {
                generate_row(tblBody, quiz[i]['question'], result[i]['answer'], "×", 'td');
            }
        }
     ...
     * @return void
     */
    function generate_row(tblBody, text1, text2, text3, $td) {
